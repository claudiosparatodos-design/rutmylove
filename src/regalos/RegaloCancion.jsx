...
import { resolverImagen, enParrafos } from '../utilidades/medios'
import { aparecer, enCascada, resorteSuave } from '../utilidades/animaciones'

/* FLOR 1 — Una canción para ti.
   La portada es el enlace: al tocarla se abre Spotify en otra pestaña. */

function PortadaDeRespaldo() {
  // Si todavía no hay portada, dibujamos una en vez de mostrar un hueco roto
  return (
...
export default function RegaloCancion() {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={aparecer} className="cancion__ficha">
        <p className="etiqueta">La canción</p>
        <h3 className="display cancion__titulo">{CANCION.titulo}</h3>
        <p className="cuerpo cancion__artista">{CANCION.artista}</p>
